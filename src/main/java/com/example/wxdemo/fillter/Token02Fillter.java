 ...
 */
public class Token02Fillter implements Filter {
    private static Logger logger = LoggerFactory.getLogger(Token02Fillter.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (uri != null && !uri.equals("")) {
            ...
            if (uri.indexOf("Token") > 0 ) {
                ...
                String token = ((HttpServletRequest) request).getHeader("token"); // 令牌
                if (StringUtils.isNotBlank(token)) {
                    try {
                        ...
                        if (valid) {
                            ...
                            response.getWriter().print(JSONObject.toJSONString(ResultData.build(401, "传入了无效的token")));
                    ...
                    response.getWriter().print(JSONObject.toJSONString(ResultData.build(401, "token是空的")));
                    response.getWriter().flush();
                    response.getWriter().close();
                    logger.warn("=================token为空=====================");
                }
